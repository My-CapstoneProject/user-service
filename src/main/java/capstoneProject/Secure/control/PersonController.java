...
public class PersonController {
    ...
    public ResponseEntity<String> registerPerson(@RequestBody Person user) {
        try {
            ...
            if (authService.isUserRegistered(user.getUsername())) {
                ...
                if ("ADMIN".equalsIgnoreCase(user.getRole())) {
                ...
                } else if ("SHOPKEEPER".equalsIgnoreCase(user.getRole())) {
                    ...
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("Invalid role. Please provide a valid role.");
                }
            }
        } catch (Exception e) {
            // Handle exceptions
    ...
    public ResponseEntity<Map<String, String>> getToken(@RequestBody Person authRequest) {
        ...
        if (user.getStatus() == 0) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("error", "You are not yet approved. Contact admin"));
        }
    ...
    public ResponseEntity<List<Person>> getUsersWaitingForApproval() {
        ...
        if (usersWaitingForApproval.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body(usersWaitingForApproval); // No Content if no users
                                                                                               // found
        }
